...
public class Problem011 {
	...
	static void solution() {
		int[][] grid = new int[][] { new int[] { 8, 2, 22, 97, 38, 15, 0, 40, 0, 75, 4, 5, 7, 78, 52, 12, 50, 77, 91, 8 },
		...
		for (int i = 0; i <= grid.length - 1; i++)
			for (int j = 0; j <= grid[i].length - 4; j++)
				ans = Math.max(ans, grid[i][j] * grid[i][j + 1] * grid[i][j + 2] * grid[i][j + 3]);
		for (int i = 0; i <= grid.length - 4; i++)
			for (int j = 0; j <= grid[i].length - 1; j++)
				ans = Math.max(ans, grid[i][j] * grid[i + 1][j] * grid[i + 2][j] * grid[i + 3][j]);
		for (int i = 0; i <= grid.length - 4; i++)
			for (int j = 0; j <= grid[i].length - 4; j++)
				ans = Math.max(ans, grid[i][j] * grid[i + 1][j + 1] * grid[i + 2][j + 2] * grid[i + 3][j + 3]);
